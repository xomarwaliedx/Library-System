<template>
  ...
  export default {
    data: () => ({
      ...
      userTypeRules: [
        value => !!value || 'Please select a user type',
      ],
      loginError: null,
    }),
    methods: {
      async submitForm() {
        try {
          const response = await axios.post('http://localhost:8080/api/user/users/login', {
            email: this.email,
          ...
          this.loginError = error.response ? error.response.data : 'Login failed. Please try again.';
